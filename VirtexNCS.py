...
def menu():
  ...
  if choice == '1' or choice == '01':
    ...
    os.system('clear')
    print (f"{logo}\n{g}INFO SCRIPT\n========================\n{p}[{y}✓{p}] {c}Author: {g}MUHAMMAD ANUGRO CAHYO RAMADHAN\n{p}[{y}✓{p}] {c}Team  : {colors()}NEWBIE {colors()}CYBER {r}SECU{w}RITY\n{p}[{y}✓{p}] {c}Script: {colors()}{os.path.basename(sys.argv[0])}\n{p}[{y}✓{p}] {c}Path  : {os.path.realpath(sys.argv[0])}\n{p}[{y}✓{p}] {c}Size  : {os.stat(os.path.realpath(sys.argv[0])).st_size} Byte\n{p}[{y}✓{p}] {c}Link  : {colors()}https://github.com/MRM05T3R/VirtexNCS\n{p}[{y}✓{p}] {c}Update: {colors()}{UPDATE}\n{p}[{y}✓{p}] {c}Versi : 1.1\n\n{g}Contact Me ^_^\n==================\n{p}[{y}✓{p}] {c}Github: {colors()}https://github.com/MRM05T3R/\n{p}[{y}✓{p}] {c}Ig.   : {colors()}https://www.instagram.com/sumoonid\n{p}[{y}✓{p}] {c}Wa.   : {colors()}+6283846681933\n{p}[{y}✓{p}] {c}Email : {colors()}famllycyber285@gmail.com\n{a}")
